...
void Skeleton::makeJoint(const string &name, const Vector3 &pos, const string &previous)
{
    ...
    if(previous == string("")) { // this skeleton doesn't have parent
      fPrevV.push_back(-1); //just make a place for shoulders
    } else { //add a bone
...
FileSkeleton::FileSkeleton(const std::string &filename)
{
    ...
    while(!strm.eof()) {
        ...
        makeJoint(line[0], p, line[4]);
    }

    // TODO: should write in the file and read
    makeSymmetric("lhipjoint", "rhipjoint");
    makeSymmetric("lfemur", "rfemur");
    makeSymmetric("ltibia", "rtibia");
    makeSymmetric("lclaivcle", "rclaivcle");
    makeSymmetric("lhumerus", "rhumerus");
    makeSymmetric("lhand", "rhand");
